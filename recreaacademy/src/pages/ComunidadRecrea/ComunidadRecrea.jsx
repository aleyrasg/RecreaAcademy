...
const categoryStyles = {
  ...
  "Anuncios Oficiales": { icon: <CampaignIcon sx={{ mr: 1, color: '#FE8A12' }} />, color: '#FE8A12' },
  "Charla Docente": { icon: <ChatIcon sx={{ mr: 1, color: '#A3A804' }} />, color: '#A3A804' },
  "Convocatorias": { icon: <HowToVoteIcon sx={{ mr: 1, color: '#1F9CD0' }} />, color: '#1F9CD0' },
};

const conversacionesMock = [
  { icon: '🎉', titulo: '¡Bienvenidas y bienvenidos a Comunidad Recrea!', autor: 'Sofía Castellanos', hace: 'hace 2 hrs', respuestas: 32, categoria: 'General' },
  { icon: '☕', titulo: '¿Qué te inspira a seguir enseñando?', autor: 'Alejandra Sánchez', hace: 'hace 2 días', respuestas: 30, categoria: 'Charla Docente' },
  { icon: '🛠️', titulo: 'Herramientas digitales que realmente te han funcionado', autor: 'Luis Castañeda', hace: 'hace 2 días', respuestas: 22, categoria: 'Anuncios Oficiales' },
  { icon: '📢', titulo: 'Convocatoria al reto de innovación educativa', autor: 'Dirección de Formación', hace: 'hace 3 días', respuestas: 18, categoria: 'Convocatorias' },
];
    ...
    <List>
      {conversaciones.map((conv, index) => (
        <React.Fragment key={index}>
          ...
          >
            <ListItemText
              primary={<Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{`${conv.icon} ${conv.titulo}`}</Typography>}
              secondary={
                <>
  ...
  return (
    <Layout>
      ...
      <Box sx={{ display: 'flex', height: '100vh' }}>
            ...
            <>
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Typography variant="h6" sx={{ color: 'inherit' }}>{selectedCategory}</Typography>
                </Box>
              ...
              <ListaConversaciones conversaciones={conversacionesFiltradas} onSelect={setConversacionSeleccionada} />
            </>
          ) : (
            <DetalleConversacion
              conversacion={conversacionSeleccionada}
              comentariosIniciales={[
                { autor: 'Sofía Castellanos', hace: 'hace 21min', texto: '👋 ¡Saludos a todos los que se suman hoy a este espacio educativo!' },
                { autor: 'Emmanuel Elizondo', hace: 'hace 22min', texto: '🙌 Bienvenidos a quienes se integran hoy a Comunidad Recrea 👋', destacado: true },
                { autor: 'Antonio Vitela', hace: 'hace 23min', texto: '¡Qué gusto ver tantas y tantos colegas por aquí!' },
                { autor: 'Alberto Sánchez', hace: 'hace 24min', texto: '🌱 ¡Cada docente que llega aporta una nueva chispa al aula digital!' },
                { autor: 'Sofía Castellanos', hace: 'hace 25min', texto: '😊 ¡Una gran bienvenida a quienes se integran por primera vez!' },
              ]}
        ...
        >
          <Box sx={{ mt: 8 }}>
            <List subheader={<ListSubheader component="div">Categorías</ListSubheader>}>
